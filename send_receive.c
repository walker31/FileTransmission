...
int Chat()
{
    ...
    printf("\n*Many People are Talking Now\n"
           "*Actually, You can only talk to One Person\n"
    ...
    who = selectOperation(1, count, 1, "Please input user No.[1]:");
...
int selectFiles(char* fullpath,  char* targetIP)
{
    ...
    ///初始化command* com的IP HERE
    com.peer.sin_family = AF_INET;
    com.peer.sin_port = htons(IPMSG_DEFAULT_PORT);
    com.peer.sin_addr.s_addr = inet_addr(targetIP);
    bzero(com.peer.sin_zero, sizeof(com.peer.sin_zero));
    ...
    fuckTheString(fileName, 0); //去除前后空白字符
    printf("finished fuckTheString().\n");

    if (fileName[0]=='\0')
        sleep(1);
    printf("finished check EOFs\n");
    ...
    printf("finished lstat()\n");

    if (S_ISREG(fileAttr.st_mode))
    {
        fileType = 1;
        printf("finished getting file attribute\n");
    }
    ...
    printf("finished initializing pointers \n");
    ...
    fileNo++;
    printf("finished preparing tasks\n");
    ...
    else
    {
        ...
        newTask->next = sendFileHead.next;
        sendFileHead.next = newTask;
...
void* sendData(void* option)
{
    ...
    for (i=0; i<4; i++)
    {
        if (readMaohao(fSock, buf, RECFRG, ':')<=0)
        {
    ...
    if (curSend != NULL)
    {
        ...
        if (curFile != NULL)
        {
            ...
            case 1: //发送文件
                ...
                printf("opened the transferring file.\n");
    ...
    if ((curErr<0) || (errno == ECONNRESET) || (errno == EPIPE))
    ...
    if (curFile!=NULL)
    {
        ...
        //设置一个记号文件，表示文件发好了
        if(NULL == fopen("sendfile.finished", "a+"))
        {
            FILE* fd;
            fd = fopen("sendfile.finished", "a+");
            sleep(1);
            fclose(fd);
        }
    }
...
int sendDir(int fSock, const char* fullpath, int fileSize, int fileType)
{
    ...
    addMaohao(strtmp, sizeof(strtmp));
    if (utf8)
...
int cancelSending()
{
    ...
    {
        pthread_mutex_lock(&sendFMutex);
        count = listReadToSend(gsList, sendFileHead.next, sizeof(gsList)/sizeof(gsList[0])-1);
        pthread_mutex_unlock(&sendFMutex);
        ...
        index = selectOperation(1, count, 1,
        ...
        if ((*(gsList+index-1))->fileSock>=0)
            shutdown((*(gsList+index-1))->fileSock, SHUT_RDWR);
        pthread_mutex_unlock(&sendFMutex);
    }
}
...
void* getData(void* option)
{
    ...
    while((cur=head->next) != NULL)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            tmp=getDir(cur, gList); //其实可以换成上次压缩再发送的做法
            break;
        default:
            tmp = -1;
    ...
    printf("Finished Getting File(s)\n");

    //设置一个记号文件，表示文件收好了
    if(NULL == fopen("getfile.finished", "a+"))
    {
        FILE* fd;
        fd = fopen("getfile.finished", "a+");
        sleep(1);
        fclose(fd);
    }
    //搞完了休息一下，退出
...
int getDir(void *option, gsNode *gList)
{
    ...
    do
    {
        ...
        readBytes = readMaohao(sockfd, hSize, HSIZE, ':');
        ...
        case IPMSG_FILE_DIR:
            ...
            if (tmp+1>=sizeof(fullPath))
            {
                printf("getDir: target directory is too long.\n"
...
int logout()
{
    ...
    sendMsg(&com);
    printf("LinPop File Support module is exiting...\n\n");
}
