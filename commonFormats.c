...
const char allHosts[] = "255.255.255.255"; //广播地址

int msgSock; //UDP发消息套接字
int fileSock; //TCP传文件套接字
struct passwd* pwd;
struct utsname sysName;
char workDir[FILENAME];
int utf8; //字符集标志位(UTF8/GB2312)

gsNode sendFileHead, getFHead; //发送和接收文件列表
...
//解析接收到的字符串
int deMsg(char *msg, int size, command* com)
{
    char *start;
    char *pos;
    char outStr[COMLEN];
...
//生成要发送的message
int initMsg(char* msg, command* com, size_t msgLen)
{
    ...
    if (com->commandNo & IPMSG_FILEATTACHOPT)
    {
        ...
        while ((curFile != NULL) && (tmp>0))
        {
            ...
            {
                printf("initMsg: get fileName error.\n");
                return -1;
            }

            addMaohao(fileName, sizeof(fileName));
    ...
    while (1)
    {
        ...
        while(1)
        {
            ...
            case 1:
                ...
                tail->fileName[min] = '\0';
                delMaohao(tail->fileName, sizeof(tail->fileName));//::  ----> :
                break;
            case 2:
